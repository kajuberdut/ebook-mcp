...
- **Metadata Extraction**: Title, author, language, publication date, and identifiers
- **Table of Contents Parsing**: Hierarchical TOC extraction as `(title, href)` pairs
- **Flexible Chapter Extraction**: Extract chapter content in clean Markdown using chapter title, case-insensitive substring, 1-based index, or raw internal href anchor link
- **Batch Processing**: Discover and process all `.epub` e-books in any library directory
...
To spin up the containerized stack along with the official **MCP Inspector** web UI:
```bash
uv run poe compose inspector
```
Open `http://localhost:6274` in your browser. For a full step-by-step tutorial on navigating the Inspector tools tab and testing tools, see the [MCP Inspector Tutorial](docs/tutorials/using_mcp_inspector.md).
...
# Get EPUB metadata
metadata = get_epub_metadata("/path/to/book.epub")

# Get table of contents
toc = get_epub_toc("/path/to/book.epub")

# Get specific chapter content (in Markdown format)
chapter_content = get_epub_chapter_markdown("/path/to/book.epub", "CHAPTER II")
```
...
#### `get_epub_metadata(epub_path: str) -> Dict[str, Union[str, List[str]]]`
Get metadata (title, author, publisher, language, identifier) from an EPUB file.
...
- `ebooklib`: EPUB file parsing and structure extraction
- `beautifulsoup4`: HTML parsing and DOM element extraction
