...
- **EPUB Chapter Pagination**: Added `start_index: int = 0` and `page_size: int = 50000` optional parameters to `get_epub_chapter_markdown` (fixing upstream Issue #11 / PR #12). Prevents character limit errors when retrieving large chapters by allowing AI models to chunk through chapter text smoothly with explicit continuation notices.
